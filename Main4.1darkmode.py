...
if IF_DOUBLE_BUFF:
    screen = pygame.display.set_mode((1366, 768), pygame.DOUBLEBUF, pygame.HWSURFACE)
else:
...
class return_life:
    ...
    def update(self):
        if self.tick < 100 and newMoveblePhoto.photolist[7].get_score() > 5000:
            self.lake += 1
            if self.lake % 18 == 0:
                self.lake = 0
                self.tick += 1
        ...
        if not self.be_used:
            if self.tick < 100 and newMoveblePhoto.photolist[7].get_score() > 15000:
                ...
                if self.lake % 40 == 0:
                    ...
                    # self.tick += 1
                self.memory_score -= 1
                if (newMoveblePhoto.photolist[7].get_score() - self.memory_score) > 200:
                    self.tick += 1
                    self.memory_score = newMoveblePhoto.photolist[7].get_score()
            elif self.tick == 100:
                pygame.event.pump()
                key_pressed_list = pygame.key.get_pressed()
                global IF_WEAPON_AUTO
            ...
            if self.tick > 1:
                ...
                if self.lake % 4 == 0:
                    ...
                    self.tick -= 1
                self.memory_score -= 1
                if (newMoveblePhoto.photolist[7].get_score() - self.memory_score) > 400:
                    self.tick += 1
                    self.memory_score = newMoveblePhoto.photolist[7].get_score()
